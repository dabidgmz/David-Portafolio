...
const Contact = () => {
  ...
  const EMAIL = 'david.gmzherrera28@gmail.com'
  const PHONE = '+52 871 141 9810'

  const handleCopy = (e, text, kind) => {
    if (e.metaKey || e.ctrlKey || e.shiftKey || e.altKey) return
    e.preventDefault()
    copy(text, kind === 'email' ? t('projects.copied') : t('projects.copied'))
  }
  ...
  const handleSubmit = async (e) => {
    ...
    if (EMAILJS_SERVICE_ID && EMAILJS_TEMPLATE_ID && EMAILJS_PUBLIC_KEY) {
      try {
        ...
        toast(t('contact.form.success'), { variant: 'success' })
        setFormData({ name: '', email: '', subject: '', message: '' })
      } catch {
        setFormStatus('error')
        toast(t('contact.form.error'), { variant: 'error' })
      } finally {
            ...
            <div className="contact-methods">
              ...
              >
                ...
                <button
                  ...
                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); copy(PHONE.replace(/\s/g, ''), t('projects.copied')) }}
                  aria-label={t('projects.copyPhone')}
                  title={t('projects.copyPhone')}
                >
                  <CopyIcon size={14} />
                </button>
              ...
              <a href={`mailto:${EMAIL}`} className="contact-method">
                ...
                <button
                  type="button"
                  className="method-copy"
                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); copy(EMAIL, t('projects.copied')) }}
                  aria-label={t('projects.copyEmail')}
                  title={t('projects.copyEmail')}
                >
                  <CopyIcon size={14} />
                </button>
              </a>
